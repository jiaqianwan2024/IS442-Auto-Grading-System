 ...
 *
 * Produces IS442-Plagiarism-Report.xlsx with two sheets:
 *
...
public class PlagiarismReportExporter {
    ...
    private void buildFlaggedSheet(Workbook wb,
        ...
        if (flagged.isEmpty()) {
            ...
            row.createCell(0).setCellValue("✅ No suspicious pairs detected.");
            return;
        }

        int rowIdx = 2;
        for (PlagiarismResult r : flagged) {
            writeResultRow(sheet.createRow(rowIdx++), r, flaggedStyle, flaggedPct);
        }
    }
    ...
    private void buildAllPairsSheet(Workbook wb,
        ...
        for (PlagiarismResult r : all) {
            writeResultRow(sheet.createRow(rowIdx++), r,
                    r.isFlagged() ? flaggedStyle : normalStyle,
                    r.isFlagged() ? flaggedPct   : pctStyle);
        }
    }
    ...
    private void writeHeader(Row row, CellStyle style) {
        String[] headers = {"Question","Student A","Student B","Similarity","Shared FPs","Flagged","Summary"};
    ...
    private CellStyle buildHeaderStyle(Workbook wb) {
        ...
        Font font = wb.createFont(); font.setBold(true); font.setFontHeightInPoints((short) 11);
        style.setFont(font);
    ...
    private CellStyle buildTitleStyle(Workbook wb) {
        CellStyle style = wb.createCellStyle();
        Font font = wb.createFont(); font.setBold(true); font.setFontHeightInPoints((short) 13);
        style.setFont(font);
        return style;
    }
}